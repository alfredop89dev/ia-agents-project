...
export default function StatusBar({
  ...
  return (
    <>
      {/* Mobile layout */}
      <div className="flex sm:hidden flex-col border-t border-border bg-surface font-mono text-xs text-subtle">
        <div className="flex items-center">
          <div className="flex items-center gap-3 px-3 py-1 min-w-0 flex-1">
            {availableModels && onModelChange ? (
              <select
                value={model}
                onChange={(e) => onModelChange(e.target.value)}
      ...
      {/* Desktop layout */}
      <div className="hidden sm:flex items-center border-t border-border bg-surface font-mono text-xs text-subtle overflow-x-auto">
        {modelSection}
        {ctxSection}
        {verbositySection}
        <div className="ml-auto">
